#The following codes produces the raw RData used in Table 1. "table_1_2_3_merge.R" uses the RData produced by "table.R" to produce Table 1 in our manuscript.
...
#Change n to obtain the results for the Tables 6 and 7 in the Appendix.
n=1000
D=200
b0=0.15
b1=0.75
...
G=10.8125+b1*x1+b2*x2+b3*x1*x2
y=G+E
yobs=y
yobs[M==0]=NA
...
for(boot in 1:D){
	...
	#Models for MR
	MRobs=cbind(bsdat$x1,bsdat$x2,bsdat$x1^2,bsdat$x2^2)
	...
	propensity.3=glm(M~MRobs[,3]+MRobs[,4],family="binomial") #estimates propensity model 3
    	...
    	augmentation.1=glm(yobs~MRobs[,1]+MRobs[,2]+MRobs[,1]:MRobs[,2],family=gaussian)
    	aug.1=(cbind(rep(1,n),MRobs[,1],MRobs[,2],MRobs[,1]*MRobs[,2])%*%as.matrix(augmentation.1$coefficients,,1))[,1] #estimates mean model 1	

	augmentation.2=glm(yobs~MRobs[,1]+MRobs[,2],family=gaussian)
    	aug.2=(cbind(rep(1,n),MRobs[,1],MRobs[,2])%*%as.matrix(augmentation.2$coefficients,,1))[,1] #estimates mean model 2
	
	augmentation.3=glm(yobs~MRobs[,3]+MRobs[,4],family=gaussian)
    	aug.3=(cbind(rep(1,n),MRobs[,3],MRobs[,4])%*%as.matrix(augmentation.3$coefficients,,1))[,1] #estimates mean model 3
	...
	psppdat=data.frame(yobs[M==1],logit_ps1_s1,Xobs[M==1,1],Xobs[M==1,2],Xobs[M==1,1]*Xobs[M==1,2],linear_basis_s1)
	...
	predpsppdat=data.frame(logit_ps0_s1,Xobs[M==0,1],Xobs[M==0,2],Xobs[M==0,1]*Xobs[M==0,2],linear_basis0_s1)
	...
	#NP
	gamdat=data.frame(yobs[M==1],Xobs[M==1,1],Xobs[M==1,2],Xobs[M==1,1]*Xobs[M==1,2])
	colnames(gamdat)=c("yobs","X1","X2","int")
	predgamdat=data.frame(Xobs[M==0,1],Xobs[M==0,2],Xobs[M==0,1]*Xobs[M==0,2])
	...
	psbppdat=data.frame(yobs[M==1],bart_ps1_s1,Xobs[M==1,1],Xobs[M==1,2],Xobs[M==1,1]*Xobs[M==1,2],linear_basis_bart_s1)
	...
	predpsbppdat=data.frame(bart_ps0_s1,Xobs[M==0,1],Xobs[M==0,2],Xobs[M==0,1]*Xobs[M==0,2],linear_basis0_bart_s1)
	...
	psppdat=data.frame(yobs[M==1],logit_ps1_s3,Xobs[M==1,1],Xobs[M==1,2],Xobs[M==1,1]*Xobs[M==1,2],linear_basis_s3)
	colnames(psppdat)=c("yobs","ps","X1","X2","int",paste0("basis",1:nknots))
	psppdat$id=1
	pspp=lme(yobs~ps+X1+X2+int,random=list(id=pdIdent(~0+basis1+basis2+basis3+basis4+basis5+basis6+basis7+basis8+basis9+basis10+basis11+basis12+basis13+basis14+basis15+basis16+basis17+basis18+basis19+basis20)),data=psppdat)
	predpsppdat=data.frame(logit_ps0_s3,Xobs[M==0,1],Xobs[M==0,2],Xobs[M==0,1]*Xobs[M==0,2],linear_basis0_s3)
	colnames(predpsppdat)=c("ps","X1","X2","int",paste0("basis",1:nknots))
	predpsppdat$id=1
	pspppred=as.numeric(predict(pspp,newdata=predpsppdat))
...
save.image(paste0("table_1_",server_num,".RData"))
